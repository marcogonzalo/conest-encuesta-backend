...
module Api
	module V1
		class ReportesController < ApplicationController
				...
				respond_to do |format|
					if error.nil?
						...
						format.csv do
							headers['Content-Disposition'] = "attachment; filename=\"reporte-consulta\""
							headers['Content-Type'] ||= 'text/csv; charset=UTF-8; header=present'
							render :reporte_materia_completo
						end
					elsif error == :no_materia	
			...
			# Devuelve la totalización de resultados de cada sección para las preguntas del instrumento aplicado en el período en una materia
				...
				if @periodo_academico.nil?
					...
					if @materia.nil?
						...
						if @oferta_periodo.nil?
							...
							@resultados = ReportePeriodo.materia_periodo(@oferta_periodo,@instrumento)
						end
					end
				end
				respond_to do |format|
					if error.nil?
						...
						format.csv do
							headers['Content-Disposition'] = "attachment; filename=\"reporte-completo-#{@materia.codigo}-periodo-#{@periodo_academico.periodo}\""
							headers['Content-Type'] ||= 'text/csv; charset=UTF-8; header=present'
							render :reporte_materia_completo
						end
					elsif error == :no_materia	
			...
			def reporte_instrumento_periodo
			end
